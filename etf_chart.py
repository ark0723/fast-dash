...
def getIndicator(df): # df(for one item): stock price or etf price 
    def return_fn(df):
        return df.pct_change().fillna(0) # pct_change() -> (다음행 - 현재행) / 현재형 -> 일일수익률
...
# load df
df = pd.read_pickle('data/etf.pkl')
...
app.layout = html.Div([
    ...
    html.Div(id='layout2', children=[
        dcc.DatePickerRange(id = 'date1',
        ...
        html.Br(),
        html.Br(),
        html.Button('Submit', id = 'btn1', n_clicks=0, style={'fontSize': '16px'})
    ]),
    ...
    html.Div(id = 'layout3', children=[
        dcc.Dropdown(id = 'two_idx',
                    options = options,
    ...
    html.Div(id='layout4', children=[
        ...
        html.Br(),
        html.Br(),
        html.Button('Submit', id = 'btn2', n_clicks=0, style={'fontSize': '16px'})
    ]),
...
def showPlot(n_clicks, col, start_date, end_date):
    ...
    layout1 = go.Layout(title = '{}'.format(col),
                        xaxis={'title':change_date_format(df.index[0])+ " ~ " + change_date_format(df.index[-1])},
                        yaxis = {'tickformat': ','}
                        )
    ...
    trace2 = go.Scatter(x = drawdown_df.index, y = drawdown_df['DrawDown'], 
                        mode = 'lines', name = f'{col} drawdown', fill='tonexty')
    layout2 = go.Layout(title = f'DrawDown of {col}',
                        xaxis={'title':change_date_format(df.index[0])+ " ~ " + change_date_format(df.index[-1])},
                        yaxis = {'tickformat': '%'}
                        )
...
def update_correlation(n_clicks, two_idx:list, start, end):
    ...
    trace = go.Scatter(x = X, y = y, mode = 'markers', 
                     marker = dict(size = 6, symbol = 'circle'), line = dict(width =2))
    
    # 3. layout
    
    layout = go.Layout(title = f'{kind1} & {kind2} Scatter Plot <br> Correlation: {correlation}',
